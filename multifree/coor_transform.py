...
def pca(x, keepdims: int=6) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    featmin = np.min(x_rotated_scaled, axis=0)
    featmax = np.max(x_rotated_scaled, axis=0)
    featrange = featmax - featmin
    return torch.tensor(x_mean), torch.tensor(eigvec), torch.tensor(variance_sqrt), torch.tensor(featrange), torch.tensor(featmin)

def minmaxscaler(features, featrange, featmin, backscale=False):
    """
    Scale the range of features to [-1,1]
    scaled_features = 2 * (features - featmin) / featrange - 1
    """
    if backscale:
        backscaled_features = featrange * ((features + 1) / 2) + featmin
        return backscaled_features
    else:
        scaled_features = (2 * (features - featmin) / featrange) - 1
...
class ZCAWhitening(nn.Module):
    """
    Implementation of the ZCA whitening and unwhitening process.
    """
    def __init__(self, dataset, inverse=False):
        super(ZCAWhitening, self).__init__()
        if dof is None:
            keepdims = dataset.shape[1]
        else:
            keepdims = dataset.shape[1] - dof
        x_mean, pc_matrix, variance_sqrt = pca(dataset, keepdims=keepdims)
        self.register_buffer("x_mean", torch.tensor(x_mean).float().requires_grad_(False))
        self.inverse = inverse
        var_sqrt_diag = torch.diag(torch.tensor(variance_sqrt))
        if inverse:
            unwhitening_matrix = torch.matmul(var_diag, torch.tensor(pc_matrix.T))
            self.register_buffer("unwhitening_matrix", unwhitening_matrix.float())
        else:
            whitening_matrix = torch.matmul(torch.tensor(pc_matrix), torch.diag(1.0/torch.tensor(variance_sqrt)))
            self.register_buffer("whitening_matrix", whitening_matrix.float())
    def forward(self, x):
        if self.inverse:
            x_transformed = torch.matmul(x, self.c00_sqrt) + self.x_mean
        else:
            x_transformed = torch.matmul(x-self.x_mean, torch.linalg.inv(self.c00_sqrt))
        return x_transformed
